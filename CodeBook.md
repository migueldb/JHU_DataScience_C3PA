...
5. Extract *Input Data* dataset zip file to a directory name stored in the variable _sourceDir_.
6. Read the *features.txt* file, pull the column with the *Input Data* dataset column labels and save it to _features_.
7. Modify the column labels in the _features_ by removing brackets, hyphens, commas, converting to lower case, and finally, replacing acronyms with more descriptive names as per tidy data principles.
...
10. Read the test data from *X_test.txt* from the *Input Data* dataset using _features_ as the vector of column names.
11. Using dplyr select the mean and standard deviation variables.
12.Using dplyr add the subject column as a factor column from *subject_test.txt* from the *Input Data* dataset.
13.Using dplyr add the activity column as a numeric column from *y_test.txt* from the *Input Data* datase (to be changed to factors later).
14. Using dplyr replace the activity column values with character activity labels from the activity codes tibble generated above modifying it to factor column making sure all the possible factors 1###are considered.
15. Read the test data from *X_train.txt* from the *Input Data* dataset using _features_ as the vector of column names.
16. Using dplyr select the mean and standard deviation variables.
17. Using dplyr add the subject column as a factor column from *subject_train.txt* from the *Input Data* dataset.
18. Using dplyr add the activity column as a numeric column from *y_train.txt* from the *Input Data* datase (to be changed to factors later).
19. Using dplyr replace the activity column values with character activity labels from the activity codes tibble generated above modifying it to factor column making sure all the possible factors 1###are considered.
20. Join the test and train data sets into _fullDS_ taking advantage the fact that the column names are exactly the same.
21. Group fullDS by _activitylabel_ and _subject_ and summarize all variables using mean and save it to _summarizedDS_
22. Write summarizedDS into a txt file
...
### NOTE:
For full details on the *Input Data* variables please refer to the links provided in the *README.md* file.